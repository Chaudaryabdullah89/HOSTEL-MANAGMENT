...
const HostelPage = () => {
  ...
  const [selectedWardenForView, setSelectedWardenForView] = useState(null);
  const [isWardenViewDialogOpen, setIsWardenViewDialogOpen] = useState(false);
  ...
  const handleAddHostel = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Refetch hostels to get updated data with wardens
  ...
  const handleDeleteHostel = async (hostelId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Refetch hostels to get updated data
        await refetchHostels();
      } else {
  ...
  const handleSelectHostelForEdit = (hostel) => {
    ...
    setWardenId(hostel.wardens?.[0]?.id || "");
    setWardensIds(hostel.wardens?.map(w => w.id) || []);
  ...
  return (
    ...
    <div className="p-2 md:p-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {hostel.wardens && hostel.wardens.length > 0 ? (
                          hostel.wardens.map((warden, index) => {
                            ...
                            return (
                              <Badge
                                ...
                                onClick={() => {
                                  setSelectedWardenForView(warden);
                                  setIsWardenViewDialogOpen(true);
                                }}
                              >
                                {warden.name}
                              </Badge>
      ...
      {/* Warden Details Dialog */}
      <Dialog open={isWardenViewDialogOpen} onOpenChange={setIsWardenViewDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="text-2xl font-semibold">Warden Details</DialogTitle>
            <DialogDescription>
              View detailed information about this warden
            </DialogDescription>
          </DialogHeader>
          {selectedWardenForView && (
            <div className="space-y-6 mt-4">
              {/* Warden Avatar/Initial */}
              <div className="flex items-center justify-center">
                <div className="w-24 h-24 rounded-full bg-blue-100 flex items-center justify-center">
                  <span className="text-4xl font-bold text-blue-600">
                    {selectedWardenForView.name?.charAt(0).toUpperCase()}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="border-b pb-3">
                  ...
                  <p className="text-lg font-semibold text-gray-900">{selectedWardenForView.name}</p>
                </div>

                <div className="border-b pb-3">
                  <h3 className="text-sm font-medium text-gray-500 mb-1">Email Address</h3>
                  <p className="text-base text-gray-900">{selectedWardenForView.email}</p>
                </div>

                <div className="border-b pb-3">
                  <h3 className="text-sm font-medium text-gray-500 mb-1">User ID</h3>
                  <p className="text-sm font-mono text-gray-600">{selectedWardenForView.id}</p>
                </div>

                {/* Find hostels this warden is assigned to */}
                <div className="pt-2">
                  <h3 className="text-sm font-medium text-gray-500 mb-2">Assigned Hostels</h3>
                  <div className="space-y-2">
                    {hostels
                      .filter(h => h.wardens?.some(w => w.id === selectedWardenForView.id))
                      .map(hostel => (
                        <div
                          key={hostel.id}
                    ...
                    {!hostels.some(h => h.wardens?.some(w => w.id === selectedWardenForView.id)) && (
                      <p className="text-sm text-gray-500 italic">No hostels assigned</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="flex justify-end gap-2 pt-4 border-t">
                <Button
                  variant="outline"
                  onClick={() => setIsWardenViewDialogOpen(false)}
                >
                  Close
                </Button>
              </div>
