...
export async function updateRoomStatusBasedOnCapacity(roomId: string) {
  try {
    ...
    console.log(
      `Room ${room.roomNumber}: ${activeBookingsCount}/${roomCapacity} capacity used`,
    );
    ...
    if (room.status !== newStatus) {
      console.log(
        `Status changed from ${room.status} to ${newStatus}, updating...`,
      );
...
export async function updateAllRoomStatuses() {
  try {
    ...
    for (const room of rooms) {
      ...
      if (room.status !== newStatus) {
        console.log(
          `Updating room ${room.roomNumber} from ${room.status} to ${newStatus}`,
        );
        await prisma.room.update({
          where: { id: room.id },
          data: { status: newStatus },
